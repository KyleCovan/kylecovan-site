...
| **56** | **Idea intake only (not a site change):** first-principles brief for a home-controlled craft that leaves the dense atmosphere and returns visuals — `docs/ideas/home-controlled-spacecraft.md`, with a pointer in `docs/post-ideas.md` and `content-queue/home-controlled-spacecraft/`. No `/builds/` page until a craft exists. Near-space balloon is Stage A; orbital PocketQube is Stage B. Standing cash constraint unchanged. |
