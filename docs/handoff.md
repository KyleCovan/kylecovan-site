...
| 38 | **Shipped.** GitHub repo `KyleCovan/kylecovan-site` (later renamed to `kylecovan-com`, §56), Cloudflare Pages, auto-deploy on push. Live at `kylecovan-site.pages.dev`. |
...
| **56** | **GitHub repo renamed `kylecovan-site` → `kylecovan-com`.** Matches the domain and the build id. Cloudflare Pages **project** stays `kylecovan-site` (so `kylecovan-site.pages.dev` is unchanged). Site docs and `package.json` updated. Onesimus is not mounted in the cloud agent — `scripts/fix_onesimus_repo_refs.py` rewrites `KyleCovan/kylecovan-site` refs there once the Acasis SSD is available, and appends a decision log entry. Post-rename checks: GitHub App still has repo access; a push to `main` still deploys; local `git remote set-url` if a clone still points at the old name. GitHub redirects the old URL. The rename itself needs admin on GitHub and is done in Settings → General (cloud tokens cannot). |
