...
src/
  ...
  pages/writing/index.astro   the writing index — "In Your Sight"
...
- **One writing door.** In Your Sight at `/writing/` lists every published
...
- **Do not revive `build:` as the thing that hides a post from In Your Sight.**
...
- **`.epigraph` is not the `h1`.** `/writing/` carries Psalm 19:14 directly
  under the h1. The verse must never become the heading; see the note in the
  CSS. The page name is **In Your Sight**.
